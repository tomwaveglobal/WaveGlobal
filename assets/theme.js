...
class ProductInfo extends HTMLElement {
  ...
  applyVariantFiltering(selectedOptionValues = []) {
    const variantDataEl = document.getElementById(`product-variants-${this.dataset.productId}`);
    if (!variantDataEl) return;
  
    const variants = JSON.parse(variantDataEl.textContent);
    const optionCount = variants[0].options.length;
  
    // Step 1: Get all selected values in order
    const currentSelection = [];
    for (let i = 0; i < optionCount; i++) {
      const selectedInput = document.querySelector(`[data-option-index="${i}"]:checked`);
      currentSelection.push(selectedInput ? selectedInput.value : null);
    }
  
    // Step 2: For each option group, filter possible valid values
    for (let i = 0; i < optionCount; i++) {
      ...
      variants.forEach(variant => {
        ...
        for (let j = 0; j < optionCount; j++) {
          if (j === i) continue;
          if (currentSelection[j] && variant.options[j] !== currentSelection[j]) {
            match = false;
            break;
          }
        }
        if (match) possibleValues.add(variant.options[i]);
      });
  
      // Step 3: Disable/hide unavailable options
      const optionInputs = document.querySelectorAll(`[data-option-index="${i}"]`);
      optionInputs.forEach(input => {
        const wrapper = input.closest('li') || input.closest('label') || input;
  
        if (['color', 'model'].includes(input.name.toLowerCase())) {
          input.disabled = false;
          wrapper.classList.remove('disabled', 'hidden');
          return;
        }
  
        if (possibleValues.has(input.value)) {
          input.disabled = false;
          wrapper.classList.remove('disabled', 'hidden');
        } else {
          input.disabled = true;
          ...
          if (input.checked) {
            input.checked = false;
            const selectedOptionLabel = wrapper.closest('.dropdown-group')?.querySelector('.selected-option');
            if (selectedOptionLabel) selectedOptionLabel.innerText = 'Select an option';
          }
        }
      });
    }

    if (typeof applySwatchDisabledStates === 'function') {
      applySwatchDisabledStates();
    }
  }
  ...
  handleOptionValueChange({ data: { event, target, selectedOptionValues } }) {
    ...
    setTimeout(() => {
      this.applyVariantFiltering(selectedOptionValues);
      // Check for elements with data-pre-order attribute
      const preOrderElements = document.querySelectorAll('[data-pre-order]');

      if (preOrderElements.length > 0) {
        // Get the actual selected variant instead of URL params
        ...
        if (selectedVariantScript) {
          try {
            ...
            if (currentVariant && currentVariant.id) {
              currentVariantId = currentVariant.id.toString();
            }
          } catch (e) {
        ...
        preOrderElements.forEach(orderMessage => {
          const preOrderValue = orderMessage.getAttribute('data-pre-order');
          if (preOrderValue === currentVariantId) {
...
/* ========================================
   Disabled Variants - Smart Combination Checking
   ======================================== */

// Variant Availability Checker - Greys out unavailable label-swatch elements based on ALL combinations
function applySwatchDisabledStates() {
  const variantPicker = document.querySelector('variant-selects');
  if (!variantPicker) return;

  const productId = variantPicker.id.split('-').pop();
  const variantsScript = document.getElementById(`product-variants-${productId}`);
  if (!variantsScript) return;

  const variants = JSON.parse(variantsScript.textContent);
  const optionWrappers = variantPicker.querySelectorAll('.variant-input-wrapper');
  
  // Get current selected options for ALL positions
  const getCurrentOptions = () => {
    const options = {};
    optionWrappers.forEach((wrapper, index) => {
      const selectedInput = wrapper.querySelector('input[type="radio"]:checked');
      if (selectedInput) {
        options[index] = selectedInput.value;
      }
    });
    return options;
  };

  // Check if a specific combination exists in the variants
  // For example: if testing Gray at position 0, and currently selected is [Black, EnergySave, 4 Person]
  // It checks if [Gray, EnergySave, 4 Person] exists
  const doesCombinationExist = (testOptionIndex, testOptionValue) => {
    const currentSelection = getCurrentOptions();
    
    // Create the test combination by replacing the value at testOptionIndex
    const testCombination = { ...currentSelection };
    testCombination[testOptionIndex] = testOptionValue;
    
    // Check if ANY variant matches this EXACT combination (all selected options must match)
    const exists = variants.some(variant => {
      let matches = true;
      
      // Check each option position
      for (const [index, value] of Object.entries(testCombination)) {
        if (variant.options[parseInt(index)] !== value) {
          matches = false;
          break;
        }
      }
  ...
  // Apply disabled states to ALL label-swatch elements based on combinations
  optionWrappers.forEach((wrapper, optionIndex) => {
    wrapper.querySelectorAll('input[type="radio"]').forEach(input => {
      const label = wrapper.querySelector(`label[for="${input.id}"].label-swatch`);
      
      // Only apply to label-swatch elements
      if (label) {
        const exists = doesCombinationExist(optionIndex, input.value);
...
// Run on page load
document.addEventListener('DOMContentLoaded', applySwatchDisabledStates);
